...
本文是独立的中文版本，不再与英文正文混排。英文版本见
[`MACOS_LOCAL_FORK_CHANGELOG.en.md`](MACOS_LOCAL_FORK_CHANGELOG.en.md)。
...
7. 增加彼此独立的中文和英文 README、来源声明、工程日志和上手指南。
8. 公开打包改用上游已经跟踪的图标；本机自制 ICNS 已排除。
9. 将八个分类提交通过 PR #1 合入公开 fork 的 `main` 分支。
...
源码清理、私有内容隔离、路径移植和提交拆分已经完成。下一阶段的重点是在另一台干净 Apple Silicon Mac 上按上手指南完成从克隆到打包的端到端验证，并继续检查共享 `Core/` 改动对 Linux、iOS、原版《将军》、录像确定性和网络锁步的影响。
