...
FontCharsClass *LoadUnicodeFallbackFont(Int size, Bool bold, const char *base_name)
{
	...
	// The localized preferred font is authoritative: for Chinese it is normally a
	// full-coverage family such as Arial Unicode MS. Trying aliases first can make
	// Fontconfig resolve "Arial Unicode" to Verdana, which loads successfully but
	// has no CJK glyphs and therefore renders hollow squares.
	...
	// Fall back to known Unicode families only if the localized choice is absent.
